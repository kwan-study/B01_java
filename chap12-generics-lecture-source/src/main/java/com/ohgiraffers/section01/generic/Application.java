...
public class Application {
    public static void main(String[] args) {
        ...
        String primitiveResult = (String)result;        // (Double)형으로 다운 캐스팅을 해야 하지만
                                                        // (String)으로 다운 캐스팅도 컴파일 시점에 가능해지고
                                                        // 이는 런타임 에러를 발생 시킨다.
                                                        // (Object를 활용한 것은 자료형이 안전하지 않다.)

//        GenericTest<Integer> gt1 = new GenericTest<Integer>();
        GenericTest<Integer> gt1 = new GenericTest<>();     // 객체의 생성자 부분에는 빈 다이아몬드 연산자 허용

//        String strResult = (String)gt1.getValue();        // 컴파일 시점에 컴파일 에러로 인해 타입 체크 X
        int intResult = gt1.getValue();                     // 다운 캐스팅을 고려하지 않아도 된다.
