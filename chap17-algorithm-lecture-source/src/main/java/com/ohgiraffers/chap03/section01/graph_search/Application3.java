...
public class Application3 {
    ...
    public static int solution(String input) throws IOException {
        ...
        /* 설명. 입력값이 공백 없이 이어져 들어옴에 따라 한 문자씩 뜯어서 int 배열(map)에 옮겨담는 반복문 */
        for (int i = 0; i < N; i++) {
            String str = br.readLine();
            char[] chars = str.toCharArray();       // 문자열 -> char[]

            for (int j = 0; j < M; j++) {
                map[i][j] = Character.getNumericValue(chars[j]);        // '1' -> 숫자 1
            }
        }
    ...
    private static void bfs(int x, int y) {
        ...
        while (!q.isEmpty()) {
            ...
            /* 설명. 상하좌우를 둘러보는 개념의 for 문(벽이 있는지, 유효한 범위인지, 지나온 길인지) */
            for (int i = 0; i < 4; i++) {
                int curX = n.x + dirX[i];
                int curY = n.y + dirY[i];
