 ...
 *   시간복잡도 O(nlogn)이지만 최악의 경우는 O(n^2)이다.
* */
public class Application4 {
    ...
    private static int process(int left, int right, int[] arr) {
        ...
        while(true) {
            ...
            /* 설명. pivot 위치에 있는 값보다 작은 값을 가르키면 lo 포인터를 증가 */
            do {
                lo++;
            } while(arr[lo] < pivot);

            /* 설명. pivot 위치에 있는 값보다 크면서 lo 포인터의 위치보다 크거나 같은 위치면 hi 포인터 감소 */
            do {
                hi--;
            } while(arr[hi] > pivot && hi >= lo);

            /* 설명. lo 포인터의 위치가 hi 포인터를 앞지르려고 하면 hi 포인터 위치를 반환 (분할 기준) */
            if (lo >= hi) {
                return hi;
            }
